---
title: "Personal Blog 1"
date: 2026-05-19
draft: false
description: "Blog for assignment 1 of the project"
slug: "blog1"
tags: ["authors", "config", "docs"]
authors:
  - "rishi_ponnapalli"
showAuthorsBadges : true
---

# Personal Blog 1
In this phase, my contributions included setting up the repository for the entire team to use as well as brainstorming ideas and writing a user persona. As the repository owner, I had to setup the repository and config the page for viewing. In addition, I also read through the docs and setup a template for all my team members to follow while they created their own personal blogs. I also helped them with this process, including using git and making pull requests. I also wrote one of our 3 user personas for the phase 1 deliverables.

This first week in Belgium has been cool. Seeing the culture in Leuven and Brussels has been a new experience, and I am glad to be here. I have also learned a lot about how the EU works theough the talks we've had, and best of all, I have had some great food!

<img src="first_mousse.JPG" alt="Mousse on First Day" style="transform: rotate(0deg); display: block; max-width: 100%;" />

<img src="liege_waffle.JPG" alt="Liege Waffle" style="transform: rotate(0deg); display: block; max-width: 100%;" />

<img src="neuhaus_mousse.JPG" alt="Mousse from Neuhaus" style="transform: rotate(0deg); display: block; max-width: 100%;" />

*Some of the food I have eaten here*